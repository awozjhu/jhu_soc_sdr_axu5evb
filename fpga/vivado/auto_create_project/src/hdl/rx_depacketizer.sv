module rx_depacketizer #(
    ...
    // Drop-beat flag (first beat after header)
    logic drop_first;
    logic last_now;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (state == HDR && in_beat) begin
                ...
                // When we just consumed word #2, next beat after header is the
                // first "payload" beat we want to drop.
                if (header_count == 2'd2)
                    drop_first <= 1'b1;
            end

            // When transitioning back to HDR, reset header counter & payload info
            ...
            // Payload decrement only on *forwarded* beats (not dropped one)
            if (state == FWD_PAYLOAD && in_beat && !drop_first) begin
                if (payload_remaining != 16'd0)
                    payload_remaining <= payload_remaining - 16'd1;
            end

            // Clear drop flag exactly when we consume that first post-header beat
            if (state == FWD_PAYLOAD && in_beat && drop_first) begin
                drop_first <= 1'b0;
            end
        end
    ...
    always_comb begin
        ...
        unique case (state)
            // ---------------- HDR: eat and drop 3 header words ----------------
            HDR: begin
                s_axis_tready = 1'b1;  // always ready for header
                if (s_axis_tvalid) begin
                    if (header_count == 2'd2) begin
                        // After 3rd header word: 0-length payload goes back to HDR
                ...
                if (!m_axis_tvalid || m_axis_tready) begin
                    ...
                    if (s_axis_tvalid) begin
                        if (drop_first) begin
                            ...
                            // send_data/send_last don't care
                        end else begin
                            send_valid = 1'b1;
                            send_data  = s_axis_tdata;

                            // last beat when:
                            //   - remaining count is 1, OR
                            //   - incoming TLAST is asserted

                            last_now  = (payload_remaining == 16'd1) || s_axis_tlast;
